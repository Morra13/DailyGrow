...
@section('content')
    <form class="register" method="post" autocomplete="off" enctype="multipart/form-data" action="{{ route(\App\Http\Controllers\Api\Auth\RegisterControllerApi::ROUTE_REGISTER) }}">
        ...
        <label class="register__label flex" for="birthday">
            <span class="register__label_text">День рождения</span>
            <input class="register__input input-reset" type="date" name="birthday" id="birthday">
        </label>
        ...
        <div class="register__buttons flex">
            ...
            <a class="register__auth btn-reset flex" href="{{ route(\App\Http\Controllers\Auth\RegisterController::ROUTE_AUTH) }}">
                Авторизация
            </a>
        </div>
